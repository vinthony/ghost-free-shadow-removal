...
if is_training:
    ...
    input_images_path=prepare_data(train_real_root,stage=['shadow']) # no reflection ground truth for real images
        ...
        for id in np.random.permutation(num_train):
            ...
            if input_images_ids[id] is None:
                ...
                if magic < ARGS.use_da: #choose from fake images
                    ...
                    iminput,imtarget,maskgt = parpare_image(current_img_id,(neww,newh),da=True,stage=['','shadow_free','shadow_mask'])
